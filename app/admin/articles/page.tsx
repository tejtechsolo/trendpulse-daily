  ...
  return (
    <main className="section container">
      ...
      {rows.length === 0 ? <div className="empty-state"><h2>No articles yet</h2><p>Create your first draft.</p><Link className="text-link" href="/admin/articles/new">Create an article →</Link></div> : (
        <div className="article-table-wrap"><table className="article-table"><thead><tr><th>Title</th><th>Category</th><th>Status</th><th>Risk</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{rows.map((article) => <tr key={article.id}><td><Link href={`/admin/articles/${article.id}/edit`}>{article.title}</Link></td><td>{article.categories?.name ?? '—'}</td><td><span className="status-badge">{article.status}</span></td><td>{article.risk_level}</td><td>{new Date(article.updated_at).toLocaleDateString('en-IN')}</td><td><div className="inline-actions"><Link className="text-link" href={`/admin/articles/${article.id}/edit`}>Edit</Link>{article.status !== 'published' && <form action={changeArticleStatus}><input type="hidden" name="id" value={article.id}/><input type="hidden" name="status" value="published"/><button className="link-button" type="submit">Publish</button></form>}</div></td></tr>)}</tbody></table></div>
      )}
    </main>
  );
}
