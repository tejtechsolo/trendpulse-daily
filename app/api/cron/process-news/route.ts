    ...
    for (const item of items ?? []) {
      ...
      try {
        ...
        const { data: article, error: articleError } = await supabase.from('articles').insert({
          title: generatedArticle.title, slug, excerpt: generatedArticle.excerpt, content: generatedArticle.content,
          category_id: category?.id ?? null, source_id: source?.id ?? null, status: 'pending_review', risk_level: risk,
          seo_title: generatedArticle.seoTitle, seo_description: generatedArticle.seoDescription, tags: generatedArticle.tags,
        }).select('id').single();
